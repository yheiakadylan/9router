...
export async function handleImageGenerationCore({
  ...
  const promptPreview = body.prompt ? `prompt="${body.prompt.slice(0, 50)}..."` : `no-prompt`;
  log?.debug?.("IMAGE", `${provider.toUpperCase()} | ${model} | Mode: ${isImg2Img ? 'Img2Img' : 'Txt2Img'} | ${promptPreview}`);
