...
async function ensureRingInitialized() {
  ...
  try {
    ...
    recentRing.items = rows.reverse().map((r) => ({
      ...
      durationMs: parseJson(r.meta, {})?.durationMs || 0,
    }));
  } catch {}
}
...
export async function getActiveRequests() {
  ...
  const recentRequests = [...recentRing.items]
    ...
    .map((e) => {
      const t = e.tokens || {};
      return {
        timestamp: e.timestamp, model: e.model, provider: e.provider || "", endpoint: e.endpoint || null,
        promptTokens: t.prompt_tokens || t.input_tokens || 0,
        completionTokens: t.completion_tokens || t.output_tokens || 0,
        status: e.status || "ok",
        durationMs: e.durationMs || e.meta?.durationMs || 0,
      };
    })
    .filter((e) => {
...
export async function saveRequestUsage(entry) {
  try {
    ...
    const metaObj = entry.durationMs ? { durationMs: entry.durationMs, ...(entry.meta || {}) } : (entry.meta || {});
    ...
    if (inserted) {
      pushToRing({ ...entry, durationMs: entry.durationMs || 0 });
...
export async function getUsageStats(period = "all") {
  ...
  const recentRows = db.all(`SELECT timestamp, provider, model, endpoint, tokens, status, meta FROM usageHistory ORDER BY id DESC LIMIT 100`);
  const seen = new Set();
  const recentRequests = recentRows
    .map((r) => {
      ...
      return {
        timestamp: r.timestamp, model: r.model, provider: r.provider || "", endpoint: r.endpoint || null,
        promptTokens: t.prompt_tokens || t.input_tokens || 0,
        completionTokens: t.completion_tokens || t.output_tokens || 0,
